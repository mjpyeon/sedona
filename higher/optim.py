...
class DifferentiableOptimizer(_abc.ABC):
    def __init__(
        ...
        **kwargs
        ...
        # Custom
        self._idx_to_param: _typing.Dict[_typing.Tuple, _typing.Any] = dict()
        ...
        for group_idx, (group, orig_group) in enumerate(zipped):
            ...
            for p_idx, p in enumerate(orig_group['params']):
                if p in other.state:
                    ...
                    self._idx_to_param[(group_idx, p_idx)] = p
                else:
                    self._idx_to_param[(group_idx, p_idx)] = None
    ...
    def print_param_shape(self, group_idx=0):
        for p in self.param_groups[group_idx]['params']: print(p.shape) if p is not None else print("None")
    ...
    def state_dict(self):
        ...
        * param_groups - a dict containing all parameter groups
        ...
        packed_state : _typing.Dict[int, _typing.Dict] = dict()
        for group_idx in range(len(self.param_groups)):
            for p_idx in range(len(self.param_groups[group_idx]['params'])):
                p = self._idx_to_param[(group_idx, p_idx)]
                if p is not None:
                    pid = id(self._idx_to_param[(group_idx, p_idx)]) if isinstance(p, _torch.Tensor) else p
                    packed_state[pid] = {
                        k:(v.detach() if isinstance(v, _torch.Tensor) else v)
                        for k, v in self.state[group_idx][p_idx].items()
                    }
        return {
            'state': packed_state,
    ...
    def step(
        ...
        override: _typing.Optional[_OverrideType] = None,
        grad_callback: _typing.Optional[_GradCallbackType] = None,
        grouped_grad_callbacks: _typing.Optional[_typing.Iterable[_GradCallbackType]] = None,
        device: _typing.Optional[_torch.device] = None,
        **kwargs
        ...
        Args:
            ...
            grouped_grad_callbacks: (optional) a list of gradient callback
                functions applied to gradients for each group
        ...
        '''
        # Modified (calculate gradients only when params are tracked by the optimizer)
        indices_visible_params = [i for group, mapping in zip(self.param_groups, self._group_to_param_list) for i in mapping]
        grad_targets = [
            p if p.requires_grad and i in indices_visible_params else _torch.tensor([], requires_grad=True)
        ...
        if grad_callback is not None:
            ...
            grad_callback(all_grads)
        elif self._grad_callback is not None:
            all_grads = self._grad_callback(all_grads)

        grouped_grads = []
        for group_idx, (group, mapping) in enumerate(zip(self.param_groups, self._group_to_param_list)):
            grads = []
            for i, index in enumerate(mapping):
                group['params'][i] = params[index] if device is None else params[index].to(device)
                if device is None or all_grads[index] is None:
                    grads.append(all_grads[index])
                else:
                    grads.append(all_grads[index].to(device))
            if grouped_grad_callbacks is not None:
                grads = grouped_grad_callbacks[group_idx](grads)
            grouped_grads.append(grads)

        self._update(grouped_grads, device)

        new_params = params[:] if device is None else [p.to(device) for p in params]
        for group, mapping in zip(self.param_groups, self._group_to_param_list):
            for p, index in zip(group['params'], mapping):
                if self._track_higher_grads:
                    new_params[index] = p if device is None else p.to(device)
                else:
                    new_params[index] = p.detach().requires_grad_() if device is None else p.detach().to(device).requires_grad_()

        if self._fmodel is not None:
            self._fmodel.update_params(new_params, device)

        return new_params


    @_abc.abstractmethod
    def _update(self, grouped_grads: _GroupedGradsType,
                device: _typing.Optional[_torch.device] = None,
                **kwargs) -> None:
        pass
            ...
            for p_idx, (p, g) in enumerate(zip(group['params'], grads)):
                ...
                if momentum != 0:
                    ...
                    if 'momentum_buffer' not in param_state:
                        ...
                        buf = param_state['momentum_buffer'] if device is None else param_state['momentum_buffer'].to(device)
        ...
        for group_idx, (group, grads) in enumerate(zipped):
            ...
            for p_idx, (p, g) in enumerate(zip(group['params'], grads)):
                ...
                if device is None:
                    ...
                    exp_avg, exp_avg_sq = state['exp_avg'].to(device), state['exp_avg_sq'].to(device)
                if amsgrad:
                    max_exp_avg_sq = state['max_exp_avg_sq'] if device is None else state['max_exp_avg_sq'].to(device)

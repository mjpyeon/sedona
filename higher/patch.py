...
def move_buffer_device(
    ...
    for key, value in fmodule._buffers.items():
        if not _torch.is_tensor(value):
            ...
            fmodule._buffers[key] = value.to(device)

    for name, child in fmodule._modules.items():
        if name in fmodule._modules:
            move_buffer_device(fmodule._modules[name], device)
        else:
            raise KeyError(
                "Did not find expected submodule "
                "{} of monkey-patched module {}.".format(name, fmodule)
...
def _update_patched_params(
    ...
    for name, child in fmodule._modules.items():
        child_params_offset = _update_patched_params(
            child, params_box, child_params_offset, device
        )
    ...
    with _modify_internally(fmodule):
        for name, param in zip(
            ...
            if device is None:
                ...
                setattr(fmodule, name, param.to(device))
...
def make_functional(
    ...
    def _update_params(self, params, device=None):
        self.fast_params = params
        params = self._expand_params(params)
        _update_patched_params(self, [params], 0, device)
        if device is not None:
            move_buffer_device(self, device)
